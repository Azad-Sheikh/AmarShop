...
namespace AmarShop.Admin
{
    public partial class AddQuantity : System.Web.UI.Page
    {
        ...
        private void BindItemName()
        {
            ...
            SqlCommand cmd = new SqlCommand("select distinct IID,IName from Item", con);
        ...
        {
            SqlConnection con = new SqlConnection(GetConnectionString());
            SqlCommand cmd = new SqlCommand("update Item set Qnt = Qnt+@Qnt where IID=@ID", con);
        ...
        private void BindGridView()
        {
            ...
            SqlCommand cmd = new SqlCommand("select IName,isnull(Qnt,0) as Qnt,isnull(AQnt,0) as AQnt,isnull(SQnt,0) as SQnt from Item ", con);
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
            con.Close();
